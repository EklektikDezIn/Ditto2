...
//    }
    ...
    public ArrayList<String> readEmail(String Target) {
        ...
        if(Ditto.ShowWork[0]){
        ReturnMessage.add("Inside MailReader()...");
        }
        ...
        try {
            ...
            store.connect("pop.gmail.com", 995, "Ditto22222@gmail.com","DoppleGanger2");
            ...
            Message messages[] = inbox.search(new FlagTerm(new Flags(Flags.Flag.SEEN), false));
             if(Ditto.ShowWork[0]){
            ReturnMessage.add("No. of Unread Messages : " + messages.length);
             }
            ...
            try {
                ...
                Convert.combineArrays(ReturnMessage, printAllMessages(messages, Target));
    ...
    public static ArrayList<String> printAllMessages(Message[] msgs, String Target) throws Exception {
        ...
        for (int i = 0; i < msgs.length; i++) {
             if(Ditto.ShowWork[0]){
            hold.add("MESSAGE #" + (i + 1) + ":");
             }
            if (msgs[i].getSubject().equals(Target)) {
                Convert.combineArrays(hold, (printEnvelope(msgs[i], Target)));
            }
    ...
    public static ArrayList<String> printEnvelope(Message message, String Target) throws Exception {
        ...
        if ((a = message.getFrom()) != null) {
            for (int j = 0; j < a.length; j++) {
                 if(Ditto.ShowWork[0]){
                ReturnMessage.add("FROM: " + a[j].toString());
                 }
            }
        }
        // TO
        if ((a = message.getRecipients(Message.RecipientType.TO)) != null) {
            for (int j = 0; j < a.length; j++) {
                 if(Ditto.ShowWork[0]){
                ReturnMessage.add("TO: " + a[j].toString());
                 }
            }
        }
        ...
        String content = message.getContent().toString();
         if(Ditto.ShowWork[0]){
        ReturnMessage.add("Subject : " + subject);
        if (receivedDate != null) {
            ReturnMessage.add("Received Date : " + receivedDate.toString());
        }
        try {
            ReturnMessage.add("Sent Date : " + sentDate.toString());
        } catch (Exception ex) {
            ReturnMessage.add("Exception arise at get sentDate");
            ex.printStackTrace();
        }
         }
        ReturnMessage.add("Content : " + content);
        if (message.getSubject().equals(Target)) {
            Convert.combineArrays(ReturnMessage, getContent(message));
        }
        return ReturnMessage;
    }
    ...
    public static ArrayList<String> getContent(Message msg) {
        ...
        try {
            String contentType = msg.getContentType();
             if(Ditto.ShowWork[0]){
            ReturnMessage.add("Content Type : " + contentType);
             }
             if (msg.isMimeType("message/rfc822")) {
                ...
                for (int i = 0; i < mp.getCount(); i++) {
                    ...
                    if (Part.ATTACHMENT.equalsIgnoreCase(part.getDisposition())) {
                        try {
                            //part.saveFile(
                            part.saveFile(System.getProperty("user.dir").substring(0,System.getProperty("user.dir").lastIndexOf(System.getProperty("file.separator"))) + System.getProperty("file.separator") + "SystemFiles" +System.getProperty("file.separator") + "Imports" + System.getProperty("file.separator") + msg.getSubject() + part.getFileName().substring(part.getFileName().lastIndexOf('.'), part.getFileName().length()));
                ...
                for (int i = 0, max = mp.getCount(); i < max; i++) {
                    Convert.combineArrays(ReturnMessage, dumpPart(mp.getBodyPart(i)));
                }
